...
public class FileUtility {
	...
	public int getNumericalDataFromExcel(String sheetName, int phoneNumber, int cellNum) throws Exception {
		FileInputStream fis = new FileInputStream(UtilityConstants.excelPath);
		Workbook wb = WorkbookFactory.create(fis);
		 double data = wb.getSheet(sheetName).getRow(phoneNumber).getCell(cellNum).getNumericCellValue();
		 int data1=(int)data;
		wb.close();
		return data1;
	}
